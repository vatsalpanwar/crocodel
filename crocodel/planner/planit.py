...
def get_simulated_data_2D_igrins_stell(T_exp = None, overheads = None, snr_array_1D = None,
                          ...
                          include_eclipse = False, include_transit = False, planet_params = None, plot_datacube = False, savedir = None ):
    ...
    for iexp in range(N_exp):
        ...
        if method == 'emission':
            if include_eclipse:
                ...
                star_and_planet_100x = stell_signal*(1. + 1e6*light_curve[iexp]*model_planet_spec_shift) ## Ftot = Fs * (1. + Fp/Fs)
            else:
                star_and_planet = stell_signal*(1. + model_planet_spec_shift) ## Ftot = Fs * (1. + Fp/Fs)
                star_and_planet_100x = stell_signal*(1. + 1e6*model_planet_spec_shift) ## Ftot = Fs * (1. + Fp/Fs)
